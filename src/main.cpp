...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
               &map_waypoints_dx,&map_waypoints_dy, &cur_lane, &ref_vel, &lane_width, &dist_min]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // START
          // Get the previouse size to avoid the collision with the car in front of us.
          ...
          // If my car is too close to the front car, the speed is reduced. 
          // If my car is not close to the front car and the speed is lower than 49.5 MPH, the speed is raised.
          // If my car is not close to the front car, but the speed is higher than and equal to 49 MPH, the current speed continues.
          if(too_close) {
            ref_vel -= .224;
          } else if (ref_vel < 49) {
            ref_vel += .224;
          ...
          // Later we will interpolate these waypoints with a spline and it in with more points that control speed.
          vector<double> ptsx;
          vector<double> ptsy;
